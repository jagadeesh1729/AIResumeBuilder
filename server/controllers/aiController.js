...
// controller to categorize a flat skills array into labeled groups via AI
// POST: /api/ai/categorize-skills
export const categorizeSkills = async (req, res) => {
    try {
        const { skills } = req.body || {};
        if (!Array.isArray(skills) || skills.length === 0) {
            return res.status(400).json({ message: 'Missing required fields: skills[]' });
        }

        const systemPrompt = "You are an expert technical resume curator. Categorize skills into concise groups suitable for a resume. Use common tech buckets (e.g., Frontend, Backend, DevOps, Cloud, Data, Mobile, Testing, Languages, Frameworks, Tools, Databases). Return JSON only as { categories: { <Category>: [skills...] } }. Keep names original, deduplicate, and place unknowns under Other.";
