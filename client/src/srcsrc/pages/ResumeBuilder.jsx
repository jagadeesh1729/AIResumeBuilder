...
import { setResume } from '../app/features/resumeSlice'
import PaddingMarginControls from '../components/PaddingMarginControls'

const ResumeBuilder = () => {
  const { resumeId } = useParams()
  const { token } = useSelector(state => state.auth)
  const { resume: resumeData, padding, margin } = useSelector(state => state.resume);
  ...
  const changeResumeVisibility = async () => {
    try {
      ...
      dispatch(setResume({ ...resumeData, public: !resumeData.public }))
        ...
        <div className=" mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <input
              ...
              onChange={(e) => dispatch(setResume({...resumeData, title: e.target.value}))}
